...
# USAGE:
# python process_fisher_data_conversation_overlap_text2semantic.py \
...
def __write_sample(dest, file_id, count, file_count, sample_rate, audio, duration, transcript, first_spk = None):
    """
    ...
    audio_path_A = os.path.join(dest, partition, f"{file_id}_{count:03}_1.wav")    
    audio_path_B = os.path.join(dest, partition, f"{file_id}_{count:03}_2.wav")
    # The 1 means the first speaker in the whole transcript sequence
    if first_spk == "A":
        audio_to_write_A = audio[:,0]
        audio_to_write_B = audio[:,1]
    elif first_spk == "B":
        audio_to_write_A = audio[:,1]
        audio_to_write_B = audio[:,0]
    wavfile.write(audio_path_A, sample_rate, audio_to_write_A)
    wavfile.write(audio_path_B, sample_rate, audio_to_write_B)
...
def __process_utterance(file_id, trans_path, line, keep_low_conf, rem_noises, emojify):
    ...
    return t_start, t_end, idx, content, channel.split(":")[0]
...
def __process_one_file(
    ...
    with open(trans_path, encoding="utf-8") as fin:
        ...
        for line in fin:
            t_start, t_end, idx, content, channel_id = __process_utterance(
            ...
            if transcript_buffer_A != [] and transcript_buffer_B != []:
                combined_data = transcript_buffer_A + transcript_buffer_B
                #print("combined_data",combined_data)
                combined_data.sort(key=lambda x: x[0])
                content_to_write =  create_overlap_content(combined_data, sep_token = '[spkchange]')
                #print("content_to_write",content_to_write)           

                final_t_start = combined_data[0][0]
                first_spk_index = combined_data[0][2]
                #print("final_t_start", final_t_start, combined_data[0])
                combined_data.sort(key=lambda x: x[1])
                final_t_end = combined_data[-1][1]
                #print("final_t_end", final_t_end, combined_data[-1])
                
                
                
                if t_start < final_t_end: # The next utterance is overlapped with our current buffer
                    if channel_id == 'A':
                        transcript_buffer_A.append((t_start, t_end, 'A', content))
                        
                    elif channel_id == 'B':
                        transcript_buffer_B.append((t_start, t_end, 'B', content))
                elif (final_t_end - final_t_start) > 40:
                ...
                else: # The next utterance is not overlapped with our current buffer
                    # Save wavs
                    audio_to_write = audio_data[floor(final_t_start * sample_rate) : ceil(final_t_end * sample_rate), :]
